...
void OnTick()
  {
   ...
   if(!CheckIfOpenOrdersByMagicNB(magicNB))//if no open orders try to enter new position
   {
      ...
      else if(Bid > bbUpperEntry && Open[0] < bbUpperEntry && rsiValue > rsiUpperLevel)//shorting
      {
         ...
         double stopLossPrice2 = NormalizeDouble(bbUpperLossExit,Digits);
         double takeProfitPrice2 = NormalizeDouble(bbLowerProfitExit,Digits);
         Print("Entry Price = " + Bid);
         Print("Stop Loss Price = " + stopLossPrice2);
         Print("Take Profit Price = " + takeProfitPrice2);
   	  
   	  double lotSize2 = OptimalLotSize(riskPerTrade,Bid,stopLossPrice2);

   	  openOrderID = OrderSend(NULL,OP_SELLLIMIT,lotSize2,Bid,10,stopLossPrice2,takeProfitPrice2,NULL,magicNB);
   	  if(openOrderID < 0) Alert("order rejected. Order error: " + GetLastError());
      }
   }
